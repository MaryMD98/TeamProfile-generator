...
describe('managerTEST', () => {
    describe("Initialization", () => {
        it("should create and object with 'name' string, 'id' number string, 'email' string and 'officeNumber' number string", () => {
        ...
        it("should throw an error if 'officeNumber' is not a number", () => {
            const cb = () => new managerTEST("mino", "1", "email@text.com", "a");
        ...
        it("should throw an error if 'officeNumber' is an empty string", () => {
            const cb = () => new managerTEST("mino", "2", "email@text.com", "");
            const err = new Error("Expected parameter 'officeNumber' to be a non-negative number");
            expect(cb).toThrowError(err);
        });
    });
